(function () {
    ...
    /**
     * STUDIOS MASTER (Unified) v2.2
     * Manual Settings Menu Injection
     */
    ...
    function startPlugin() {
        ...
        function addMenuButtons() {
            ...
            Object.keys(SERVICE_CONFIGS).forEach(function (sid) {
                if(!Lampa.Storage.get('studios_show_' + sid, true)) return; // Check storage
        ...
        // Цей код гарантовано додасть кнопку в меню налаштувань
        Lampa.Settings.listener.follow('open', function (e) {
            
            // 1. Вставка пункту в Головне меню налаштувань
            if (e.name == 'main') {
                var item = $(`<div class="settings-param selector" data-type="button" data-static="true" data-name="studios_settings">
                    <div class="settings-param__name">Налаштування Студій</div>
                    <div class="settings-param__descr">Вибір сервісів для меню</div>
                </div>`);

                item.on('hover:enter', function () {
                    Lampa.Settings.open('studios_settings');
                });

                // Спробуємо вставити після "Інтерфейсу" або в кінець
                var target = e.body.find('[data-name="interface"]');
                if(target.length) target.after(item);
                else e.body.append(item);
            } 
            
            // 2. Рендер нашої сторінки налаштувань
            else if (e.name == 'studios_settings') {
                var controller = e.body;
                
                // Заголовок
                controller.append(`<div class="settings-param__title">Відображення в меню</div>`);

                Object.keys(SERVICE_CONFIGS).forEach(function(sid){
                    var conf = SERVICE_CONFIGS[sid];
                    var isActive = Lampa.Storage.get('studios_show_' + sid, true);

                    // Використовуємо внутрішній метод Лампи для створення тогла
        ...
        // Оновлення меню після закриття налаштувань
